...
show_kali_install_method() {
    ...
    要退出Kali环境，只需输入  exit 
    要退出iSH应用，只需再次输入  exit (注意：在ipad的iSH输入里  exit 可以退出，但是在iPhone的iSH里输入  exit 不会退出而是重启iSH）
    "
}
...
install_kali() {
    ...
    if [[ "$confirm" =~ ^([Yy]|)$ ]]; then
        ...
        # 提示用户下载Kali系统文件
        echo -e "${CYAN}\n请手动下载Kali Linux根文件系统：${NC}"
        echo -e "1. 打开Safari浏览器"
        echo -e "2. 下载Kali Linux根文件系统压缩包"
        echo -e "3. 将下载的文件移动到iSH的kaliapp目录"
        echo -e "${YELLOW}\n按Enter键继续...${NC}"
        read
...
update_script() {
    ...
    if [ "$VERSION" != "$REMOTE_VERSION" ]; then
        ...
        # 备份当前脚本
        echo -e "${YELLOW}正在备份当前脚本...${NC}"
        cp -f "./$SCRIPT_NAME" "./${SCRIPT_NAME}.bak"
        ...
        # 下载(克隆)最新脚本
        git clone --depth=1 https://github.com/XiaoZhang-qd/iSH-Kali.git ~/iSH-Kali
        if [ $? -ne 0 ]; then
            echo -e "${RED}克隆仓库失败，请检查网络连接或仓库地址是否正确${NC}"
            echo -e "${YELLOW}正在恢复备份的脚本...${NC}"
            cp -f "./${SCRIPT_NAME}.bak" "./$SCRIPT_NAME"
            return 1
        fi
        # 复制新脚本到当前目录
        cp -f ~/iSH-Kali/iSH-Kali-install.sh ./
        if [ $? -eq 0 ]; then
            # 重新赋予执行权限
            chmod +x "./$SCRIPT_NAME"
            ...
            echo -e "${RED}脚本更新失败，请手动从仓库复制脚本${NC}"
            echo -e "${YELLOW}正在恢复备份的脚本...${NC}"
            cp -f "./${SCRIPT_NAME}.bak" "./$SCRIPT_NAME"
            return 1
        fi
...
check_update() {
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}获取远程版本号失败，请检查网络连接${NC}"
        # 提供备选方案
        echo -e "${YELLOW}您也可以手动访问GitHub仓库查看最新版本: https://github.com/XiaoZhang-qd/iSH-Kali${NC}"
        return 1
    fi
    ...
    while true; do
        ...
        case $choice in
            ...
            3)  # 安装Kali + iSH-Tools + 快捷指令的工具
                ...
                # 验证iSH-Tools安装
                echo -e "${YELLOW}验证iSH-Tools安装...${NC}"
                if command -v iSH-Tools &> /dev/null; then
                    echo -e "${GREEN}iSH-Tools安装成功${NC}"
                else
                    echo -e "${RED}iSH-Tools安装失败${NC}"
                fi
